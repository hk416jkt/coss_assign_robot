...
#include "circle_interfaces/msg/cmd.hpp"
#include "circle_interfaces/msg/ecd.hpp"
...
public:
                   ...
                   angular_input_(1500)
    {
        ...
        ecd_pub_ = this->create_publisher<circle_interfaces::msg::Ecd>("/ecd", loop_rate_);
        serial_pub_ = this->create_publisher<std_msgs::msg::UInt8MultiArray>("/serial_write", loop_rate_);

        cmd_sub_ = create_subscription<circle_interfaces::msg::Cmd>(
...
private:
    void cmd_callback(const circle_interfaces::msg::Cmd::SharedPtr msg)
    {
    ...
    void serial_callback(const std_msgs::msg::UInt8MultiArray::SharedPtr msg)
    {
        ...
        if (comma_pos != std::string::npos)
        {
            ...
            try
            {
                auto ecd_msg = circle_interfaces::msg::Ecd();
...
private:
    rclcpp::Publisher<circle_interfaces::msg::Ecd>::SharedPtr ecd_pub_;
    rclcpp::Publisher<std_msgs::msg::UInt8MultiArray>::SharedPtr serial_pub_;
    rclcpp::Subscription<circle_interfaces::msg::Cmd>::SharedPtr cmd_sub_;
